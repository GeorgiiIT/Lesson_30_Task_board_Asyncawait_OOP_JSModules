...
export const API = `https://62bc1534eff39ad5ee1c1d1c.mockapi.io/`;
 ...
 taskForm.addEventListener(`submit`, async e=>{
	...
	try{
		let participants = [...participantsSelect.selectedOptions]
			...
			let modifiedParticipants = await Promise.all(addedPost.participants.map(participants => controller(API+`/participants/${participants.id}`, `PUT`, {task: true})));
